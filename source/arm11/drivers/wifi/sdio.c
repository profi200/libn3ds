...
u32 SDIO_init(void)
{
	...
	TOSHSD_initPort(port, 0);
	TOSHSD_startInitClock(port, INIT_CLOCK);
	TIMER_sleepTicks(2 * TOSHSD_CLK2DIV(INIT_CLOCK) * 74);

	// This CMD does nothing for SDIO cards but it's used for detecting SPI mode.
	u32 res = TOSHSD_sendCommand(port, SDIO_GO_IDLE_STATE, 0);
	...
	//res = TOSHSD_sendCommand(port, SDIO_IO_SEND_OP_COND, 0); // Arg = 0 is probing OCR.
	...
	while(1)
	{
		res = TOSHSD_sendCommand(port, SDIO_IO_SEND_OP_COND, SDIO_OP_COND_ARG);
		if(res != 0) return SDIO_ERR_IO_SEND_OP_COND;
	...
	res = TOSHSD_sendCommand(port, SDIO_SEND_RELATIVE_ADDR, 0);
	...
	res = TOSHSD_sendCommand(port, SDIO_SELECT_CARD, rca);
...
u32 SDIO_io_rw_direct(const bool write, const u8 func, const u32 addr, const u8 in, u8 *const out)
{
	...
	ToshsdPort *const port = &g_sdioDev.port;

	const u32 arg = write<<31 | func<<28 | (write && out != NULL ? 1u<<27 : 0) | addr<<9 | in;
	const u32 res = TOSHSD_sendCommand(port, SDIO_IO_RW_DIRECT, arg);
...
/*u32 SDIO_io_rw_extended(const bool write, const u8 func, const u32 addr, const bool incAddr, u8 *buf, const u16 count, const u16 size)
{
	...
	const u32 res = TOSHSD_sendCommand(port, SDIO_IO_RW_EXTENDED(true), arg);
	if(res != 0) return SDIO_ERR_IO_RW_DIRECT;
